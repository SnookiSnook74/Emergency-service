...
class Zone {
    ...
    // Для упрощения логика написана так , чтобы растояние считалось от центра фигуры(зоны)
    func distanceTo(accident: AccidentInput) -> Double {
        let accidentCoords = accident.coordinates.split(separator: ";").compactMap { Int($0) }
        guard accidentCoords.count == 2 else { return Double.greatestFiniteMagnitude }

        let point = (x: accidentCoords[0], y: accidentCoords[1])

        switch type {
        case .circle:
            return distanceFromCircleCenter(point)
        case .triangle:
            return distanceFromPolygonCenter(point, vertices: 3)
        case .quadrilateral:
            return distanceFromPolygonCenter(point, vertices: 4)
        default:
    ...
    private func distanceFromCircleCenter(_ point: (x: Int, y: Int)) -> Double {
        let components = coordinates.split(separator: " ")
        guard let centerPart = components.first?.split(separator: ";").compactMap({ Int($0) }),
              centerPart.count == 2
        else {
            return Double.greatestFiniteMagnitude
        }
        let center = (x: centerPart[0], y: centerPart[1])
        return distanceBetweenPoints(point1: center, point2: point)
    }

    private func distanceFromPolygonCenter(_ point: (x: Int, y: Int), vertices: Int) -> Double {
        let vertexStrings = coordinates.split(separator: " ")
        guard vertexStrings.count == vertices else { return Double.greatestFiniteMagnitude }

        let sum = vertexStrings.reduce((x: 0, y: 0)) { currentSum, vertexString -> (x: Int, y: Int) in
            let coords = vertexString.split(separator: ";").compactMap { Int($0) }
            guard coords.count == 2 else { return currentSum }
            return (x: currentSum.x + coords[0], y: currentSum.y + coords[1])
        }

        let center = (x: sum.x / vertices, y: sum.y / vertices)
        return distanceBetweenPoints(point1: center, point2: point)
    }

    private func distanceBetweenPoints(point1: (x: Int, y: Int), point2: (x: Int, y: Int)) -> Double {
